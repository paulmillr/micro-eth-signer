...
export const authorization: TRet<AuthorizationHelpers> = /* @__PURE__ */ deepFreeze({
   ...
   */
  getHash(req: AuthorizationRequest): TRet<Uint8Array> {
    // Safe to validate here: every structurally valid EIP-7702 authorization contains
    // exactly 20 address bytes; invalid chain, nonce, or signature values are separate.
    if (!addr.isValid(req.address)) throw new Error('address checksum does not match');

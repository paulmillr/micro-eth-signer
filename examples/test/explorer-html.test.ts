...
/*
UI tests for examples/explorer.html, driven in headless Chromium (playwright-core).
...
(runnable ? describe : describe.skip)('explorer.html', () => {
  ...
  beforeAll(async () => {
    ...
    await page.goto(`${server.origin}/examples/explorer.html?rpc=${server.origin}/rpc`);
  });
  afterAll(async () => {
    await browser?.close();
    await server?.close();
  ...
  should('load an address: balances and transaction rows', async () => {
    ...
    // the ots+logs quick view decodes token movements, discovery included
    const amounts = await page.$$eval('#rows td.amount', (tds: any[]) =>
      tds.map((td) => td.textContent)
    );
    ok(
      amounts.some((a: string) => a.includes('USDC')),

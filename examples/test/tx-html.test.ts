...
/*
UI tests for examples/tx.html, driven in headless Chromium (playwright-core).
...
(runnable ? describe : describe.skip)('tx.html', () => {
  ...
  beforeAll(async () => {
    ...
    await page.goto(`${server.origin}/examples/tx.html?rpc=${server.origin}/rpc`);
  });
  afterAll(async () => {
    await browser?.close();
    await server?.close();
  ...
  should('create an ERC-20 transfer of the selected token', async () => {
    ...
    ok(fields.includes(CAT), 'token contract shown');

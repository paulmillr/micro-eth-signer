...
export const OURS = /* @__PURE__ */ deepFreeze({
        ...
        display: {
            ...
            formats: {
                ...
                'swapExactTokensForTokens(uint256 amountIn, uint256 amountOutMin, address[] path, address to, uint256 deadline)': {
                    intent: 'Swap',
                    interpolatedIntent: 'Swap exact {amountIn} for at least {amountOutMin}. Expires at {deadline}',
                    fields: [
                        {
                            path: 'amountIn',
                ...
                'swapTokensForExactTokens(uint256 amountOut, uint256 amountInMax, address[] path, address to, uint256 deadline)': {
                    intent: 'Swap',
                    interpolatedIntent: 'Swap up to {amountInMax} for exact {amountOut}. Expires at {deadline}',
                    fields: [
                        {
                            path: 'amountOut',
                            $ref: '$.display.definitions.tokenReceive',
        ...
        display: {
            ...
            formats: {
                'tradeWithHintAndFee(address src, uint256 srcAmount, address dest, address destAddress, uint256 maxDestAmount, uint256 minConversionRate, address platformWallet, uint256 platformFeeBps, bytes hint)': {
                    intent: 'Swap',
                    interpolatedIntent: 'Swap {srcAmount} for up to {maxDestAmount} to recipient {destAddress} with platform fee {platformFeeBps}',
                    fields: [
                        {
                            path: 'srcAmount',
                            label: 'Amount to Send',
        ...
        display: {
            ...
            formats: {
                'exactInputSingle((address tokenIn, address tokenOut, uint24 fee, address recipient, uint256 deadline, uint256 amountIn, uint256 amountOutMinimum, uint160 sqrtPriceLimitX96) params)': {
                    intent: 'Swap',
                    interpolatedIntent: 'Swap exact {params.amountIn} for at least {params.amountOutMinimum}. Expires at {params.deadline}',
                    fields: [
                        {
                ...
                'exactOutputSingle((address tokenIn, address tokenOut, uint24 fee, address recipient, uint256 deadline, uint256 amountOut, uint256 amountInMaximum, uint160 sqrtPriceLimitX96) params)': {
                    intent: 'Swap',
                    interpolatedIntent: 'Swap up to {params.amountInMaximum} for exact {params.amountOut}. Expires at {params.deadline}',
                    fields: [
                        {
 ...
 * ```
 */
export const addTokens = (repo, tokens, chainId = 1) => {
    const out = { ...repo };
    ...
    const candidates = (files) => Object.keys(files).filter((name) => {
        const c = (files[name].context || {}).contract;
        return c && !(c.deployments && c.deployments.length) && !c.factory;
    });
    const own = candidates(out);
    const bundled = candidates(ERCS);
    ...
    for (const k of Object.keys(tokens)) {
        ...
        if (t.name)
            token.name = t.name;
        if (t.symbol || t.ticker)
            token.ticker = t.symbol || t.ticker;
        if (t.decimals !== undefined)
            token.decimals = t.decimals;
        const deployments = [{ chainId: id, address }];
        const bind = (name, ctx) => {
            const clone = JSON.parse(JSON.stringify(out[name] || ERCS[name]));
        ...
        if (tag === 'erc20' && (out[permit] || ERCS[permit]))
            bind(permit, 'eip712');
    }
    return out;
};
//# sourceMappingURL=clearsig-repo.js.map
import { createDecimal, deepFreeze } from "../utils.js";
...
export const ABI = /* @__PURE__ */ deepFreeze(/* @__PURE__ */ _abi());
const meta = (opt) => {
    const info = opt.contractInfo;
    if (!info || info.decimals === undefined || !info.symbol)
...
export const hints = {
    Approval(v, opt) {
        const m = meta(opt);
        return `Allow ${v.spender} spending up to ${createDecimal(m.decimals).encode(v.value)} ${m.symbol} from ${v.owner}`;
    },
    Transfer(v, opt) {
        const m = meta(opt);
        return `Transfer ${createDecimal(m.decimals).encode(v.value)} ${m.symbol} from ${v.from} to ${v.to}`;
    },
};
// Keep default export on a distinct value; `ABI` is also a type under verbatimModuleSyntax.
const ERC20ABI = /* @__PURE__ */ deepFreeze(/* @__PURE__ */ addHints(ABI, hints));

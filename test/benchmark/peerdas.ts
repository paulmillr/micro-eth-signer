...
export async function main() {
  ...
  await mark('init(ckzg)', () => ckzg.loadTrustedSetup(PRECOMPUTE));
  await mark('init(noble)', () => (meth = new kzg.KZG(s_fast)));
  // Compute cells
  {
    console.log('computeCells:');
    const t = VECTORS.compute_cells.valid[0];
    const blobBytes = hexToBytes(strip0x(t.input.blob));
    await mark('- ckzg', () => ckzg.computeCells(blobBytes));
    await mark('- noble', () => meth.computeCells(t.input.blob));
  }
  // compute_cells_and_kzg_proofs
  {
    ...
    await mark('- noble (first)', () => meth.computeCellsAndProofs(t.input.blob));
  ...
  // recover_cells_and_kzg_proofs
  {
    ...
    await mark('- ckzg', () => ckzg.recoverCellsAndKzgProofs(t.input.cell_indices, cells));
    await mark('- noble', () => meth.recoverCellsAndProofs(t.input.cell_indices, t.input.cells));
  }
